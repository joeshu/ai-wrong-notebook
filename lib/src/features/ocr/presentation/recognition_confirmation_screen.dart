...
class _RecognitionConfirmationScreenState
  ...
  Set<String> _requiredFields(QuestionRecord record) {
    final required = <String>{};
    if ((record.ocrConfidence ?? 0) < .85 || _stemController.text.trim().isEmpty) {
      required.add(RecognitionReviewField.stem);
    }
    if (_optionsController.text.trim().isNotEmpty && (record.ocrConfidence ?? 0) < .85) {
      required.add(RecognitionReviewField.options);
    }
    if (_answerController.text.trim().isNotEmpty && (record.ocrConfidence ?? 0) < .85) {
      required.add(RecognitionReviewField.studentAnswer);
    }
    final text = '${_stemController.text}\n${_optionsController.text}';
    if (_looksFormulaDamaged(text)) required.add(RecognitionReviewField.formulas);
    return required;
  }

  bool _looksFormulaDamaged(String text) {
    final dollars = RegExp(r'\$').allMatches(text).length;
    return dollars.isOdd || text.contains(r'\\frac{') && !text.contains('}');
  }

  bool _highConfidenceSafe(QuestionRecord record) =>
      (record.ocrConfidence ?? 0) >= .85 &&
      _stemController.text.trim().isNotEmpty &&
      !_looksFormulaDamaged('${_stemController.text}\n${_optionsController.text}') &&
  ...
  Widget build(BuildContext context) {
    ...
    _initialize(record);
    final required = _requiredFields(record);
    final canProceed = required.every(_confirmed.contains) ||
        (_autoConfirm && _highConfidenceSafe(record));
    return Scaffold(
      appBar: AppBar(
  ...
  Future<void> _retryRecognition(QuestionRecord record, {String? field}) async {
    ...
    try {
      ...
      _error = '重新识别失败：$error。可继续手动录入并保留草稿。';
    } finally {
  ...
  Future<void> _saveDraft(QuestionRecord record) async {
    final updated = record.copyWith(
      normalizedQuestionText: _stemController.text.trim(),
      studentAnswer: _answerController.text.trim(),
      contentStatus: ContentStatus.needsConfirmation,
    );
    await ref.read(questionRepositoryProvider).saveDraft(updated);
    ref.read(currentQuestionProvider.notifier).state = updated;
    if (mounted) context.go('/notebook');
  }
}
